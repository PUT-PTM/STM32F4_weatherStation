...
// If an adderss is specified (<addressLength> != 0) a write without a stop
...
int I2C_ReadTransfer(u08 dev_addr, u08 *buffer, int cnt, u32 ptr, u08 ptrlen)
{
  ...
  if (ptrlen != 0)
  {
    if ((rc = I2C_start(Open_I2C, dev_addr, I2C_Direction_Transmitter))==OK)
    {
      for (i=1; i<=ptrlen; i++)
      {
...
int I2C_CheckDevice(u08 addr)
{
  ...
  //-----------------------------------------------------------------------------
  I2C_Send7bitAddress(Open_I2C, addr, I2C_Direction_Receiver);
  timeout = I2C_TIMEOUT;
  ...
  //-----------------------------------------------------------------------------
  I2C_AcknowledgeConfig(Open_I2C, DISABLE);
  while(!I2C_CheckEvent(Open_I2C, I2C_EVENT_MASTER_BYTE_RECEIVED))
  {
    if((timeout--)==0)   // wait while reading data from device
    {
      I2C_GenerateSTOP(Open_I2C, ENABLE);
      return ERROR;
    }
...
//  UART_GoToNewLine();  // ensure new line
//  printf("TIMEOUT: %s\n",msg);
  return -1;
}
